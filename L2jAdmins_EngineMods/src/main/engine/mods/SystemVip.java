...
public class SystemVip extends AbstractMods
{
	...
	public boolean onAdminCommand(L2PcInstance player, String chat)
	{
		...
		switch (command)
		{
			...
			case "removevip":
			{
				...
				removeVip((L2PcInstance) player.getTarget());
				return true;
			}
			// only for admins
			// format: setVip days
			case "setvip":
			{
				...
				L2PcInstance vip = (L2PcInstance) player.getTarget();
				...
				// Informed admin
				player.sendPacket(new CreatureSay(player.getObjectId(), SayType.TELL, "player: " + vip.getName() + "is Vip now", ""));
				// Informed player
				vip.sendPacket(new CreatureSay(player.getObjectId(), SayType.TELL, "Dear " + vip.getName() + " your are now Vip", ""));
	 ...
	 * @param dayTime
	 */
	private static void informeExpireVip(L2PcInstance player)
	{
	...
	public void addVip(L2PcInstance player, long dayTime)
	{
		// Shedule to end VIP
		...
		{
			player.setNameColor(ConfigData.VIP_NCOLOR);
		}
		
		if (ConfigData.ALLOW_VIP_TCOLOR)
		{
			player.setTitleColor(ConfigData.VIP_TCOLOR);
		}
		
		player.broadcastUserInfo();
	}
	
	public static void removeVip(L2PcInstance player)
	{
	...
	public void getAllPlayerVips(L2PcInstance player, int page)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);

...
public class StatsPlayer extends AbstractMods
{
	...
	{
		private Map<BonusType, LinkedHashMap<StatsType, Integer>> _stats = new LinkedHashMap<>();
		...
		private void initBonus()
		{
			...
			{
				for (StatsType sts : StatsType.values())
				{
			...
			for (BonusType bt : BonusType.values())
			{
				...
				if (values != null)
				{
					for (String split : values.split(";"))
					{
						...
						StatsType stat = StatsType.valueOf(parse[0]);
	...
	public boolean onCommunityBoard(L2PcInstance player, String command)
	{
		...
		if (event.equals("modified"))
		{
			...
			StatsType stat = StatsType.valueOf(st.nextToken());
			...
			String parse = "";
			for (Entry<StatsType, Integer> map : _classStats.get(classId).getAllBonus(bonusType).entrySet())
			{
	...
	private static void htmlIndexClass(L2PcInstance player)
	{
	...
	private static void htmlIndex(L2PcInstance player, ClassId classId, BonusType bonusType, int page)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.COMUNITY_TYPE);
		hb.append(Html.HTML_START);
		hb.append("<br>");

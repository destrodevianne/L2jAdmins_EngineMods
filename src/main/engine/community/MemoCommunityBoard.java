...
import net.sf.l2j.gameserver.model.items.Item;
import net.sf.l2j.gameserver.model.items.ItemEtcItem;
import net.sf.l2j.gameserver.model.items.ItemWeapon;
import net.sf.l2j.gameserver.model.items.enums.CrystalType;
import net.sf.l2j.gameserver.model.items.enums.EtcItemType;
import net.sf.l2j.gameserver.model.items.enums.ItemType2;
...
public class MemoCommunityBoard extends AbstractMods
{
					...
					if (st.hasMoreTokens())
					{
						...
						if (ih != null)
						{
							...
							if (ih.getOwnerId() == player.getObjectId())
							{
								...
								player.getInventory().addItem("auction buy", itemBuy, player, true);// se entrega al player
							}
							else
							{
								hb.append("bypass? xD");
	...
	// XXX HTML -----------------------------------------------------
	private static String getMyAuctions(L2PcInstance player)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.COMUNITY_TYPE);
		...
		for (Entry<Integer, AuctionItemHolder> entry : ph.getAuctionsSell().entrySet())
		{
			...
			hb.append("<tr>");
			hb.append("<td width=32>", Html.newImage(IconData.getIconByItemId(item.getId()), 32, 32), "</td>");
	...
	private String sellItem(L2PcInstance player, int itemObjId, int itemCount, int itemPriceCount, int itemPriceId)
	{
		...
		if (!player.getInventory().destroyItemByItemId("start auction", ConfigData.COMMISION_FOR_START_SELL_ID, ConfigData.COMMISION_FOR_START_SELL, player, false))
		{
		...
		if (item != null)
		{
			...
			AuctionItemHolder ash = new AuctionItemHolder(id, player.getObjectId(), itemObjId, item.getId(), itemCount, item.getEnchantLevel(), itemPriceCount, itemPriceId);
	...
	private String buyItem(L2PcInstance player, PlayerHolder phOwner, int key)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.COMUNITY_TYPE);
		...
		else
		{
			...
			hb.append("<tr>");
			hb.append("<td fixwidth=32>", Html.newImage(IconData.getIconByItemId(item.getId()), 32, 32), "</td>");
			hb.append("<td align=center>");
	...
	private String buyItemConfirm(L2PcInstance player, PlayerHolder phOwner, int key)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.COMUNITY_TYPE);
		if (!phOwner.getAuctionsSell().containsKey(key))
		{
		...
		else
		{
			...
			int countBuy = player.getInventory().getItemCount(ih.getItemPriceId(), -1);
			...
			player.getInventory().addItem("auction buy", itemBuy, player, true);// se lo entregamos al player
			
			L2PcInstance owner = L2World.getInstance().getPlayer(phOwner.getObjectId());
	...
	private static String getSellList(L2PcInstance player, int page)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.COMUNITY_TYPE);
		...
		List<ItemInstance> list = player.getInventory().getAvailableItems(false);
	...
	private static void giveAuctionSold(L2PcInstance owner, AuctionItemHolder ih)
	{
		...
		// mensajes
		owner.sendPacket(new CreatureSay(0, SayType.TELL, "Auction", "Has vendido el item " + item.getName()));

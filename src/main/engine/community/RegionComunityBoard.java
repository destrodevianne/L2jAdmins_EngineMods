...
import net.sf.l2j.gameserver.model.actor.base.Sex;
...
public class RegionComunityBoard extends AbstractMods
{
	...
	public boolean onCommunityBoard(L2PcInstance player, String command)
	{
		if (command.startsWith("_bbsloc"))
		{
			...
			hb.append("<br>");
			// head
			hb.append(Html.htmlHeadCommunity("TOTAL ONLINE: " + L2World.getInstance().getAllPlayers().size()));
			hb.append("<br>");
			// body
			hb.append("<center>");
			...
			for (L2PcInstance pc : L2World.getInstance().getAllPlayers())
			{
				...
				hb.append("<td fixwidth=100 align=center>", pc.getName(), pc.getName().equals("fissban") ? Html.newFontColor("FF0000", "   ADMIN") : pc.getAccessLevel() > 0 ? Html.newFontColor("LEVEL", "   GM") : "", "</td>");

...
public class VoteReward extends AbstractMods
{
	...
	private static void checkVoteRewards(int voteTop, TopType topName)
	{
		...
		Broadcast.announceToOnlinePlayers(" @ " + topName.name().toLowerCase() + ":  " + voteTop + " votes.");
		...
		if (nextVote < voteTop)
		{
			Broadcast.announceToOnlinePlayers(" @ " + topName.name().toLowerCase() + ": You won the Reward Vote!");
			
			giveRewardAllPlayers(reward, topName);
		}
		else
		{
			Broadcast.announceToOnlinePlayers(" @ " + topName.name().toLowerCase() + ": next reward in " + nextVote + " votes.");
		}

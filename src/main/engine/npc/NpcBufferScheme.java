...
import net.sf.l2j.gameserver.network.serverpackets.SetupGauge.SetupGaugeType;
...
{
	private static final int NPC_ID = 14050;
		...
		switch (bypass)
		{
			...
			case "buffpet":
			{
				if (checkTimeOut(player))
				{
					...
					if (TIME_OUT)
					{
						addTimeout(player, SetupGaugeType.GREEN, TIME_OUT_TIME / 2, 600);
					}
				}
				rebuildMainHtml(player);
				return;
			...
			case "heal":
			{
				if (checkTimeOut(player))
				{
					...
					if (TIME_OUT)
					{
						addTimeout(player, SetupGaugeType.BLUE, TIME_OUT_TIME / 2, 600);
					}
				}
				rebuildMainHtml(player);
				return;
			...
			case "removeBuffs":
			{
				if (checkTimeOut(player))
				{
					...
					if (TIME_OUT)
					{
						addTimeout(player, SetupGaugeType.RED, TIME_OUT_TIME / 2, 600);
					}
				}
			...
			case "cast":
			{
				if (checkTimeOut(player))
				{
					...
					if (TIME_OUT)
					{
						addTimeout(player, SetupGaugeType.CYAN, TIME_OUT_TIME, 600);
					}
				}
				rebuildMainHtml(player);
				return;
			...
			case "giveBuffs":
			{
				...
				if (checkTimeOut(player))
				{
					if (!FREE_BUFFS)
					{
						...
						{
							showText(player, "Sorry", "You don't have the enough items:<br>You need: <font color=LEVEL>" + cost + " " + getItemNameHtml(CONSUMABLE_ID) + "!", false, "0", "0");
							return;
						}
					}
					...
					if (TIME_OUT)
					{
						addTimeout(player, SetupGaugeType.CYAN, TIME_OUT_TIME / 10, 600);
					}
				}
	...
	private void addTimeout(L2PcInstance player, SetupGaugeType gaugeColor, int amount, int offset)
	{
	...
	private void rebuildMainHtml(L2PcInstance player)
	{
	...
	private static void showText(L2PcInstance player, String type, String text, boolean buttonEnabled, String buttonName, String location)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);
	 ...
	 * @return
	 */
	private static void createScheme(L2PcInstance player)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);
		hb.append("<html><head><title>", TITLE_NAME, "</title></head><body>");
		hb.append(Html.headHtml("BUFFER"));
	...
	private void deleteScheme(L2PcInstance player)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);
		hb.append("<html><head><title>", TITLE_NAME, "</title></head><body>");
		hb.append(Html.headHtml("BUFFER"));
	...
	private void editScheme(L2PcInstance player)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);
		hb.append("<html><head><title>", TITLE_NAME, "</title></head><body>");
		hb.append(Html.headHtml("BUFFER"));
	...
	private void getOptionList(L2PcInstance player, String scheme)
	{
	...
	private static void buildHtml(L2PcInstance player, BuffType buffType, int page)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);
		hb.append("<html><head><title>", TITLE_NAME, "</title></head><body>");
		hb.append("<center><br>");
	 ...
	 * @param action
	 */
	private void viewAllSchemeBuffs(L2PcInstance player, String schemeName, String page, String action)
	{
		HtmlBuilder hb = new HtmlBuilder(HtmlType.HTML_TYPE);
		hb.append("<html><head><title>", TITLE_NAME, "</title></head><body>");
		hb.append(Html.headHtml("BUFFER"));
